...
const PDFMiniViewers = (() => {
    "use strict";
    /*
     * PDFMiniViewers global variables.
     */
    let CMAPS;
    const DEBOUNCE_FUNCS = {};
    const DEBOUNCE_TIMER = {};
     ...
     */
    var debounce = (func, delay = 250) => {
        const hash = `F${getStringHash(func.toString())}`;
    ...
    var getFilename = (mini) => {
        ...
        const filename = viewer.dataset.pdf;
    ...
    var loadPage = (viewer, PDFPageProxy) => {
     ...
     /*   PDFPageProxy.getTextContent().then(
        ...
        */
     /*   PDFPageProxy.getTextContent().then(
            textLayerInstance.render.bind(null, textLayer, PDFPageProxy.streamTextContent(), viewport),
            (error) => {
                console.error(error);
            }
        );
        */
         ...
         PDFPageProxy.getTextContent().then((textContent) => {
            const textLayerInstance = new pdfjsLib.TextLayer({
                textContent: textContent,
                textContentStream: PDFPageProxy.streamTextContent(),
                container: textLayer,
                viewport: viewport
            });
            textLayerInstance.render();
        }).catch((error) => {
            console.error(error);
        });
   ...
   /*  var renderTextLayer = (textLayer, stream, viewport, textContent) => {
        ...
        // Assuming your TextLayer class has a render method
        textLayerInstance.render();
    };
     */
    /**
     ...
     */
    var uid = () => (Date.now().toString(36) + Math.random().toString(36).substr(2, 6)).toUpperCase();
